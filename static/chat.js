(() => {
  ...
  function addMessage(m) {
    ...
    const row = document.createElement("div"); row.className = "msg-row " + (m.author === USERNAME ? "mine" : "other");
    const bubble = document.createElement("div"); bubble.className = "bubble" + (m.kind === "sticker" ? " sticker-bubble" : "");
    const meta = document.createElement("div"); meta.className = "meta"; meta.textContent = `${m.author} • ${m.time}`; bubble.appendChild(meta);
    if (m.kind === "sticker") {
      const img = document.createElement("img"); img.className = "chat-sticker"; img.src = m.sticker_url; img.alt = `Figurinha enviada por ${m.author}`; bubble.appendChild(img);
    } else {
      const text = document.createElement("div"); text.className = "msg-text"; text.textContent = m.text; bubble.appendChild(text);
    }
    row.appendChild(bubble); messagesEl.appendChild(row); messagesEl.scrollTop = messagesEl.scrollHeight;
  }

  async function loadMessages() {
    if (loadingMessages) return; loadingMessages = true;
    try {
  ...
  form.addEventListener("submit", async e => {
    ...
    try {
      const r = await fetch(`/api/messages/${ROOM}`, {method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text,device_id:NossaSala.deviceId})});
      const data = await r.json(); if (!r.ok) throw new Error(data.error || "Não foi possível enviar."); await loadMessages();
    } catch (err) { input.value = text; status(err.message,true); }
  });
  ...
  async function sendSticker(token){try{const r=await fetch(`/api/sticker-message/${ROOM}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({token,device_id:NossaSala.deviceId})});const data=await r.json();if(!r.ok)throw new Error(data.error||"Falha ao enviar.");await loadMessages();toolDrawer.classList.add("hidden");}catch(err){status(err.message,true);}}
